...
Version : 3.0
Statut : Consolidé — fusion v2.0 (documentaire) + v2.1 (pipeline) + entrées prioritaires
Rôle : Couche d'interprétation documentaire destinée à harmoniser le comportement des IA dans les workflows FACTORY.
Documents de référence : BIB_[THEME].txt — MDE_A3 — REGLES_UNIVERSELLES — FACTORY_QA_RULES — QUIZ_ASSEMBLY_RULES
...
Lien explicite entre un angle interrogeable (identifié dans l'ANGIPREGEN) et le pool de génération auquel il est affecté.

L'angle_assignment garantit qu'un angle donné n'est utilisé que dans le pool prévu, évitant les collisions documentaires entre pools.

## Important

- un angle non assigné ne peut pas être généré,
- un angle assigné à un pool est interdit dans tous les autres,
- les assignations sont définies dans POOLS_[THEME].txt via la clé SOURCE.

## Comportement IA attendu

- respecter les assignations définies dans POOLS_[THEME],
- ne jamais utiliser un angle dans un pool autre que celui défini,
- signaler toute ambiguïté d'assignation avant génération.
...
Vérification de la fluidité et du rythme documentaire d'une partie. Contrôle l'alternance des types de questions et la progression de la difficulté.

## Ce que vérifie ce contrôle

- absence de séquences monotones (3 questions de même type d'affilée),
- progression cohérente de la difficulté,
- alternance entre joueurs, matchs, records, anecdotes, nations, époques.

## Niveau de priorité

SOFT_WARNING — non bloquant, révision possible.
...
Vérification de la densité de répétition documentaire dans une même partie ou un même stock. Détecte les surreprésentations de joueurs, nations, époques ou éditions.

## Ce que vérifie ce contrôle

- surreprésentation d'un joueur,
- surreprésentation d'une nation,
- surreprésentation d'une époque,
- surreprésentation d'une édition.
...
Vérification de l'équilibre entre thèmes, nations, époques et types de questions dans une partie.
...
Réponse incorrecte proposée dans un QCM aux côtés de la bonne réponse. Chaque question contient 3 distracteurs. Un distracteur doit être crédible, cohérent avec l'univers de la bonne réponse, et ne pas induire en erreur de façon injuste.

## Caractéristiques d'un bon distracteur

- même périmètre que la bonne réponse (géographique, temporel, thématique),
- crédible sans être correct,
- ni trop évident ni trop absurde,
- pas de répétition du même distracteur à plus de 5% dans le stock global.

## Important

- un distracteur fictif (nom inventé) nécessite une validation humaine obligatoire,
- un distracteur trop éloigné de la bonne réponse facilite artificiellement la question,
- un distracteur trop proche peut créer une ambiguïté injuste.

## Comportement IA attendu

- proposer des distracteurs du même registre que la bonne réponse,
- éviter les distracteurs absurdes, hors catégorie ou inférieurs à la réponse,
- signaler tout distracteur fictif avant intégration,
- ne jamais valider seul un distracteur fictif.
...
- l'équilibre N1/N2/N3 adapté au thème,
- le public cible,
...
Chaîne complète de traitement documentaire et de génération quiz au sein de la QUIZZZ FACTORY.

## Phases standard

```
BIB → BIPREGEN → ANGIPREGEN → POOLS → Génération → QA → Assemblage → Export
```

## Important

- chaque phase produit des livrables définis,
- aucune phase ne peut être sautée,
- la validation humaine est obligatoire entre les phases clés.
...
Pool dans lequel un angle, un item ou un fait ne doit pas être réutilisé. Les pools interdits sont définis dans POOLS_[THEME].txt via la clé EXCLUDED_POOLS.

## Important

- un angle assigné à un pool est implicitement interdit dans tous les autres,
- les interdictions explicites concernent les cas à risque élevé de collision,
- toute violation d'un FORBIDDEN_POOL constitue un POOL_COLLISION.

## Comportement IA attendu

- vérifier les exclusions avant toute génération,
- ne jamais générer une question sur un angle interdit dans le pool en cours,
- signaler les conflits d'exclusion détectés.

---

# IF-ROT (INCONTOURNABLES ROTATIFS)

## Définition

Type de pool à rotation lente. Couvre les incontournables de second rang — reconnaissables par le public cible mais moins centraux que les IF-SF.

## Paramètres fixes

- 3 pools par quiz,
- 12 questions cible par pool,
- rotation lente entre sessions.

## Important

- les IF-ROT assurent l'onboarding fluide des joueurs réguliers,
- leur contenu doit rester accessible au public cible,
- les angles IF-ROT ne peuvent pas être réutilisés dans les QV.

---

# IF-SF (INCONTOURNABLES SEMI-FIXES)

## Définition

Type de pool à rotation très lente. Couvre les références les plus emblématiques du thème — reconnues immédiatement par tout le public cible.

## Paramètres fixes

- 2 pools par quiz,
- 8 questions cible par pool,
- rotation très lente (quasi-fixe entre sessions).

## Important

- les IF-SF définissent l'identité forte du quiz,
- une finale IF-SF ne peut jamais réapparaître dans un QV,
- les angles IF-SF sont protégés et ne peuvent être réassignés.
...
Question appartenant aux pools IF-SF ou IF-ROT. Couvre les références les plus emblématiques du thème. Se distingue des Questions Variables (QV) par une rotation plus lente et une sélection éditoriale plus stricte.

## Important

- un incontournable doit être reconnaissable par le grand public cible,
- la sélection des incontournables est une décision éditoriale humaine,
- les incontournables ne peuvent pas migrer vers des pools QV.

## Comportement IA attendu

- traiter les incontournables avec une rigueur documentaire maximale,
- ne proposer comme incontournable que des références dont la notoriété est établie,
- signaler tout doute sur l'accessibilité grand public d'un incontournable proposé.
...
Niveaux de difficulté documentaire utilisés dans toute la FACTORY pour classer les items et équilibrer les stocks de questions.

- **N1** — Accessible. Reconnaissable par le grand public sans expertise particulière. Références très connues, faits notoires.
- **N2** — Intermédiaire. Nécessite une connaissance modérée du thème. Références connues des amateurs.
- **N3** — Expert. Réservé aux connaisseurs du thème. Références pointues, faits peu connus du grand public.

## Répartitions indicatives

| N1 | N2 | N3 |
|---|---|---|
| 35–45% | 35–45% | 15–25% |

Fourchettes adaptables selon le thème et le public ciblé.

## Important

- les fourchettes sont indicatives et non des contraintes absolues,
- le niveau d'un item peut être reclassé lors du rééquilibrage documentaire,
- N1/N2/N3 sont des niveaux documentaires, pas des niveaux de difficulté rédactionnelle.

## Comportement IA attendu

- viser une répartition globalement cohérente avec les fourchettes indicatives,
- signaler les déséquilibres importants entre niveaux,
- ne pas forcer un reclassement uniquement pour atteindre une fourchette cible.
...
- la pertinence généraliste dépend du thème et du public ciblé.
...
Unité opérationnelle de génération contenant un stock cible de questions d'un même type ou sous-thème. Le moteur de jeu pioche une seule question par pool à chaque partie.

## Paramètres fixes

- 20 pools par quiz (fixe pour tous les thèmes),
- 1 question piochée par pool par partie,
- 3 types de pools : IF-SF / IF-ROT / QV.

## Structure des 20 pools

| Type | Pools | Stock cible par pool | Total |
|---|---|---|---|
| IF-SF | 2 | 8 questions | 16 |
| IF-ROT | 3 | 12 questions | 36 |
| QV | 15 | 15 questions | 225 |
| **Total** | **20** | | **277** |

## Important

- le nombre de pools est fixe — 20 pour tous les quiz sans exception,
- les questions d'un même pool doivent être cohérentes entre elles,
- un pool ne doit pas contenir de doublons ni de collisions internes,
- la définition des 20 pools s'effectue à l'étape A4 via POOLS_[THEME].txt.

## Comportement IA attendu

- travailler pool par pool, jamais globalement,
- respecter le stock cible de chaque pool,
- signaler toute collision détectée entre questions d'un même pool.
...
Situation où un même fait documentaire majeur apparaît dans plusieurs pools de manière problématique, créant un risque de répétition perceptible pour le joueur.

## Formes fréquentes

- même angle utilisé dans deux pools différents,
- même fait central exploité sous deux formulations dans deux pools,
- finale IF-SF réutilisée dans un QV.

## Niveau de priorité

HARD_BLOCKER — bloque l'export final selon RULE_PRIORITY_MATRIX.

## Comportement IA attendu

- vérifier les réservations et exclusions avant toute génération,
- ne jamais réutiliser un angle réservé hors de son pool assigné,
- signaler immédiatement toute collision détectée.
...
Statut final de validation qualité d'une génération. Attribué à l'issue des contrôles définis dans FACTORY_QA_RULES.

## Valeurs standard

- **PASS** — génération conforme, export autorisé,
- **WARNING** — anomalies non bloquantes détectées, révision recommandée,
- **FAIL** — anomalies bloquantes détectées, export interdit.

## Important

- un QA_STATUS=FAIL bloque l'export final (HARD_BLOCKER),
- un QA_STATUS=WARNING permet l'export avec validation humaine,
- un QA_STATUS=PASS ne dispense pas de la validation humaine finale.
...
Questions générées sans validation finale, sans audit complet et pas encore intégrées dans le xlsx. Produit de l'étape B2 du workflow FACTORY.

Les questions brutes passent ensuite par l'audit B5 avant intégration définitive. Sur 300 questions brutes générées, environ 277 sont retenues après audit.

## Important

- les questions brutes ne sont jamais considérées comme finales,
- elles peuvent contenir des erreurs, ambiguïtés ou doublons,
- la génération se fait pool par pool, jamais globalement.

## Comportement IA attendu

- générer les questions brutes pool par pool,
- ne pas auto-valider les questions brutes,
- signaler les doutes de formulation ou d'exactitude pendant la génération.
...
Phase d'orchestration consistant à construire une partie finale de 20 questions à partir des pools validés. Intègre l'équilibrage thématique, le rythme, la courbe de difficulté et les règles anti-monotonie.

## Objectifs

- partie variée, fluide, équilibrée et rejouable,
- absence de collisions documentaires perceptibles,
- progression cohérente de la difficulté,
- alternance des types de questions.

## Règles principales

Voir `QUIZ_ASSEMBLY_RULES.md` pour les règles complètes.

---

# QV (QUESTIONS VARIABLES)

## Définition

Type de pool à rotation rapide. Cœur documentaire du système, offrant la plus grande variété thématique.

## Paramètres fixes

- 15 pools par quiz,
- 15 questions cible par pool,
- rotation rapide entre sessions.

## Important

- les QV couvrent l'ensemble de la variété documentaire du thème,
- un angle réservé à un IF-SF ou IF-ROT ne peut pas être réutilisé en QV,
- la diversité des 15 pools QV est une priorité éditoriale.
...
Intervention visant à corriger un déséquilibre documentaire devenu problématique pour la cohérence globale ou la variété quiz.
...
Élément documentaire (item, angle ou fait) réservé à un pool spécifique et interdit aux réutilisations dans d'autres pools. Défini dans POOLS_[THEME].txt via la clé RESERVED_ITEMS.

## Important

- un item réservé ne peut générer de questions que dans son pool assigné,
- la réservation est une décision éditoriale — elle protège les angles les plus forts,
- toute violation d'une réservation constitue un POOL_COLLISION.

## Comportement IA attendu

- vérifier les réservations avant chaque génération,
- ne jamais utiliser un item réservé hors de son pool assigné,
- signaler tout conflit entre réservations et assignations.
...
Volume total de questions validées disponibles pour un quiz dans le xlsx. Le stock est le produit final de l'audit B5, prêt pour l'assemblage et l'export.

## Paramètres fixes

- stock cible : 277 questions par quiz,
- généré à partir de 300 questions brutes (marge d'audit : ~23 questions),
- réparti sur 20 pools selon la structure IF-SF / IF-ROT / QV.

## Important

- le stock ne contient que des questions validées par l'humain,
- une question rejetée en audit ne compte pas dans le stock,
- le stock est distinct des questions brutes.

## Comportement IA attendu

- ne jamais considérer les questions brutes comme faisant partie du stock,
- respecter les stocks cibles par pool lors de la génération,
- signaler si le stock d'un pool risque d'être insuffisant après audit.
